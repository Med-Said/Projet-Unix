...
until [[ $reponse == "n" || $reponse == "N" ]]; do
	select choix in "Consulter la base de données" "Consulter les informations d'une personne" "Ajouter une personne" "Supprimer une personne"
	do
		case $choix in
			...
			"Consulter les informations d'une personne" )
				echo "vous avez besoin du nom pour pouvoire acceder a l'informations de cette personne !!!"
				read -p "nom : " nom
				awk -F";" -v nom=$nom '$2 == nom { print $0 }' Personnes
				break;
				;;
